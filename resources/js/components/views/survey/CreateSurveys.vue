  ...
  <div class="flex ml-[20%] min-h-screen bg-gradient-to-tr from-[#f4f4f4] to-[#f9fbfd]">
    ...
    <main class="flex-1 flex items-center justify-center p-10">
      ...
      <div class="relative w-full max-w-5xl p-10">
        ...
        <!-- Glow inside form -->
        <div class="absolute inset-0 z-0 rounded-1xl blur-3xl opacity-40
        ...
        <div class="flex justify-center mb-12">
          <div v-for="step in 4" :key="step" class="flex flex-col items-center mx-6">
            <div :class="[currentStep === step-1 ? 'bg-[#4072bc]' : 'bg-gray-300', 'rounded-full w-10 h-10 flex items-center justify-center text-white font-bold text-lg mb-1']">
              {{ step }}
            </div>
            <div class="text-sm font-medium">{{ stepTitles[step-1] }}</div>
          </div>
        </div>

        <!-- Steps Content -->
        <div v-if="currentStep === 0">
          <!-- CSV Upload with Dashed Border -->
          <div class="mb-8 border-2 border-dashed border-[#59a8f7] bg-[#f9fbfd] rounded-xl p-6 text-center">
            ...
            <input type="file" accept=".csv" @change="handleCSVUpload"
                   class="mx-auto text-sm file:rounded-lg file:border-none file:bg-[#59a8f7] file:text-white file:py-2 file:px-4 cursor-pointer"/>
        ...
        <div v-if="currentStep === 2">
          <!-- Questions -->
          <div v-for="(question, index) in form.questions" :key="index" class="bg-[#f9fbfd] border border-[#e3e3e3] rounded-xl p-6 mb-6">
            <div class="flex justify-between items-center mb-4">
              <h3 class="font-semibold text-[#4072bc] text-lg">Question {{ index + 1 }}</h3>
              <button @click="removeQuestion(index)" class="text-red-500 text-sm hover:underline">Remove</button>
            </div>
            <div class="mb-4">
            ...
            <div v-if="question.questionType === 'multiple-choice'">
              ...
              <div v-for="(option, optIndex) in question.options" :key="optIndex" class="flex gap-2 mb-2">
                ...
                <button @click="removeOption(index, optIndex)" class="text-red-400 hover:text-red-600 text-sm rounded-full">Remove</button>
              </div>
              <button @click="addOption(index)" class="bg-gray-200 px-4 py-2 rounded-full font-medium text-sm">+ Add Option</button>
            </div>
          </div>
        ...
        <div class="flex justify-between items-center mt-12">
          ...
          <button v-if="currentStep < 3" @click="nextStep" class="w-44 py-3 rounded-full bg-gradient-to-r from-[#4072bc] to-[#59a8f7] text-white font-semibold shadow-lg hover:brightness-110 transition">Next</button>
          <button v-if="currentStep === 3" :disabled="loading" @click="createSurvey"
            class="w-44 py-3 bg-gradient-to-r from-[#f07ba3] to-[#c4a8e3] hover:from-[#8475d2] hover:to-[#a7c8f8] text-white font-semibold rounded-full shadow-lg hover:brightness-110 transition">
...
// CSV Upload Logic (parse locally)
